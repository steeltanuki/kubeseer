...
example, this installs `0.1.1` (source tag `v0.1.1`):
...
`ghcr.io/steeltanuki/kubeseer:0.1.1`. The `.tgz` chart archive is a temporary
...
```sh
helm show chart oci://ghcr.io/steeltanuki/charts/kubeseer --version 0.1.1
helm template kubeseer oci://ghcr.io/steeltanuki/charts/kubeseer \
  --version 0.1.1 --namespace kubeseer-system \
...
`image.tag: ""` resolves to `Chart.appVersion` (`0.1.1` in this package).
...
`vMAJOR.MINOR.PATCH`, such as `v0.1.1`. Prerelease tags are not supported by
...
KUBESEER_PACKAGE_IMAGE_TAG=0.1.1 \
  make test-package-compatibility
```
