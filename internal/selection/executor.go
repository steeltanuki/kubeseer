...
type ResourceLister interface {
	List(context.Context, ReadTarget, metav1.ListOptions) (*unstructured.UnstructuredList, error)
}
...
func NewDynamicResourceLister(client dynamic.Interface) *DynamicResourceLister {
	return &DynamicResourceLister{client: client}
}

// List addresses exactly target.GVR and applies a namespace only for a
// discovery-reported namespaced target.
func (l *DynamicResourceLister) List(ctx context.Context, target ReadTarget, options metav1.ListOptions) (*unstructured.UnstructuredList, error) {
...
// The caller must supply an AuthorizedPlan produced by Bind.
func (e *Executor) Execute(ctx context.Context, authorized AuthorizedPlan) SelectionOutcome {
	...
	for _, target := range plan.Targets() {
		resources, err := e.listTarget(ctx, plan, target)
		if err != nil {
			outcome.Resources = nil
			outcome.Err = err
			return outcome
...
func (e *Executor) listTarget(ctx context.Context, plan SelectionPlan, target ReadTarget) ([]SelectedResource, *SelectionError) {
	...
	for {
		...
		response, err := e.lister.List(ctx, target, metav1.ListOptions{
