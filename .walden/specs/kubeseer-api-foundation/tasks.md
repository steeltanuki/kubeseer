...
status: approved
approved_at: 2026-08-26T08:17:00Z
last_modified: 2026-08-26T08:17:00Z
approved_fingerprint: sha256:2734e293f356392f6fbc43d380abe1fbcd523c581e90ed0b6a8ecfe06cdc2139
source_design_approved_at: 2026-08-26T08:11:37Z
source_design_fingerprint: sha256:d14df41fae131cbca93a9213d19723ecb566673ba6b0daa2616e39fa0ba87cd3
---
...
- [x] 1. Establish the generated Kubeseer API contract
  - [x] 1.1 Bootstrap the pinned Go toolchain and implement the versioned API package, scheme registration, typed spec/status envelopes, markers, generated deep-copy code, and focused serialization tests
    ...
    - Design: Toolchain And Module Boundary; Versioned API Package; API Registration; Root Resources; Spec And Source Envelope; Status And Result Envelope; Envtest-owned API contract checks
    - Verification:
      - command: ["sh", "-c", "GOCACHE=/tmp/kubeseer-api-foundation-go-build GOMODCACHE=/tmp/kubeseer-api-foundation-go-mod make test-api"]
  ...
  - [x] 1.2 Generate and commit the structural CRD, then add typed static contract tests for identity, scope, served/storage flags, requiredness, source validation, status schema, security boundaries, and API-evolution guards
    ...
    - Design: Generated CRD; Schema And Serialization Rules; API Evolution Strategy; Security Considerations; Installed CRD contract checks
    - Verification:
      - command: ["sh", "-c", "GOCACHE=/tmp/kubeseer-api-foundation-go-build GOMODCACHE=/tmp/kubeseer-api-foundation-go-mod make test-api"]
        expect_output: "API_CONTRACT=kubeseer-v1alpha1 STATUS=passed"
        timeout: 20m
  ...
  - [x] 1.3 Implement stable generation commands and a read-only verification path that regenerates Go and CRD artifacts in a temporary copy and fails on tracked drift
    ...
    - Verification:
      - command: ["sh", "-c", "GOCACHE=/tmp/kubeseer-api-foundation-go-build GOMODCACHE=/tmp/kubeseer-api-foundation-go-mod make verify"]
...
- [x] 2. Prove the API contract against supported Kubernetes API servers
  - [x] 2.1 Implement the parameterized envtest contract suite and a `make test-api` entry point, then prove CRD establishment, valid persistence, structural rejection, unserved-version handling, and status isolation on Kubernetes 1.35.6
    ...
    - Verification:
      - command: ["sh", "-c", "GOCACHE=/tmp/kubeseer-api-foundation-go-build GOMODCACHE=/tmp/kubeseer-api-foundation-go-mod make test-api KUBERNETES_VERSION=1.35.6"]
        expect_output: "API_CONTRACT=kubeseer-v1alpha1 STATUS=passed"
        timeout: 20m
  ...
  - [x] 2.2 Add the pinned Kubernetes 1.35.6/1.36.2 compatibility target and CI matrix, reusing the same contract suite and emitting one deterministic success marker
    ...
    - Verification:
      - command: ["sh", "-c", "GOCACHE=/tmp/kubeseer-api-foundation-go-build GOMODCACHE=/tmp/kubeseer-api-foundation-go-mod make test-compatibility"]
