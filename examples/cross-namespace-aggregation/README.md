...
Two Deployments in separate namespaces are selected and their integer replica
counts are summed in a deterministic aggregate. Apply, inspect, verify, and clean up only this entry
with:
...
The expected public outcome is one deterministic `sum` aggregate with
contributors from both exact namespaces.
